...
- [~] **T-230 — Pierwsze rankingi.** 🟡 **Pierwszy wpis LIVE 2026-08-05 (SUV-y, #406121).** Kolejność zmieniona wobec pierwotnej (terenówki → pickupy → vany): zaczynamy od **SUV-ów (6 280/mc, największa fraza, 172 modele po naszej stronie)** — decyzja z sesji 08-04/08-05 po pomiarze DFS. Dalej: terenówki i 4×4 (3 450/mc, nikt w PL tego nie obsługuje — ale API nie rozróżnia konstrukcji ramowej, filtr trzeba złożyć po naszej stronie), pickupy (2 480), najtańsze chińskie samochody (910), elektryki-bestsellery (1 880 — API nie ma filtra „tylko BEV", do rozstrzygnięcia). Definicje: `scripts/kb/rankingi.json`. 4–5 h.
