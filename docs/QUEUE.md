...
> Aktualizacja: 2026-09-07 (**T-223 ZDJĘTE, T-243 ODŁOŻONE, T-248 ma przewoźnika — decyzje Janka 07.09 wieczorem**: T-223 rozjazd depozytu zdjęty z kolejki; T-243 dystrybucja na social odłożona, bo prowadzimy ją ręcznie i kontrolujemy dashboardem pomiaru; T-248 — przesyłki chodzą **Maerskiem**, szablon śledzenia do potwierdzenia w przeglądarce, automat armatora zostaje na parkingu jako T-254. ⚠️ **T-121 NIE jest gotowy do startu mimo wpisu niżej** — pomiar 07.09: POS PayU oddaje wszystkie 19 metod jako `DISABLED`, czyli blokada z 31.08 trwa; bramka jest włączona, więc klienci są odbijani. Wcześniej tego samego dnia: **T-221 i T-244 ZDJĘTE Z KOLEJKI 07.09 — decyzja Janka**; wpisy zostają w pliku jako kontekst, oznaczone `[~]`. Wcześniej 2026-09-04: **T-248 przedefiniowany + T-253 wyceniony 04.09** — numer przesyłki bez automatu armatora: pole w karcie, mail do klienta bez linków zewnętrznych, numer w panelu klienta w kroku 5 kreatora (nie czeka na T-204); automat Maerska wydzielony do **T-254** na parking. Oba taski idą **jednym wejściem** — ten sam `class-asiaauto-order-admin.php`; pakiet wykonawczy: `docs/sesje/2026-09-04-PROMPT-T248-T253-numer-przesylki-i-termin.md`. Razem **5,5–6,5 h realnie / 14–18 h rynkowo**. Wcześniej 2026-09-02: **T-249 dopisane 02.09** — dokumenty w strefie klienta: wymiana plików Ruslan ↔ klient + lista oczekiwanych dokumentów; kształt uzgodniony z Jankiem — **jedna bryła po T-204**, 45–60 h realnie; przy okazji pomiar: **68 załączników zamówień oddaje się bez logowania** z `uploads/contracts/`. Wcześniej 2026-08-20: **T-246 WDROŻONY 20.08** — dostępność WCAG: axe 266 → 0 na 6 stronach, fokus 17 → 0, baner zgód domknięty; **2 z 7 kart audytu okazały się fałszywe** (N-4, N-7), N-2 niepotwierdzony — czeka reaudyt w `auranet`. Wcześniej 2026-08-07: **T-243 SPEC 07.08, ale ZAPARKOWANY** — dystrybucja treści na social, parasol 8 pozycji, 89–119 h, zużyte 4 h; instrukcja dostępów u Ruslana, **czekamy**; **T-241 wchłonięty** jako faza c. **T-244 i T-245 dopisane 07.08** — dwa defekty zgłoszone w pakiecie T-209 z 06.08, które nigdy nie trafiły do kolejki; T-244 zweryfikowany na produkcji. **Priorytet: T-121 i T-209 — oba mają gotowe pakiety wykonawcze.** Wcześniej: **T-121 ODBLOKOWANY 06.08** — creds produkcyjne PayU, pakiet wykonawczy gotowy, 27–36 h; **T-220 DONE 06.08** — aneks VIN, v0.34.22; **T-242 DONE 06.08** — edycja VIN w zamówieniu, v0.34.21; **T-229 DONE 05.08** — mechanizm rankingów; **T-230 — 7 rankingów LIVE**; **T-238 DONE 05.08** — ceny i tytuły hubów; **T-239 DONE 06.08** — promocja Terenwizja na stronie głównej; **T-240 zaparkowany** — indywidualizacja hubów z własnymi materiałami; **T-241** — procedura „nowe materiały z sesji → hub + reklamy", pierwszy przebieg Leopard 5 na 3 h. Wcześniej: T-217 WDROŻONY 30.07 — v0.34.14; T-219 ZAMKNIĘTY 03.08)
...
|---|---|---|---|
| **T-221** — pakiet prawny pod PayU | ⚪ **ZDJĘTE Z KOLEJKI 2026-09-07 (decyzja Janka)** | nie blokowało już T-121; 3 zadania porządkowe nie wchodzą | — |
...
| **T-223** — rozjazd depozytu: strona vs §4 umowy | ⚪ **ZDJĘTE Z KOLEJKI 2026-09-07 (decyzja Janka)** — wcześniej: czekało na Ruslana (punkt 8 w liście weryfikacyjnej) | wynik decyduje, czy ruszamy §4 wzorca (strefa krucha) | 1–4 h |
...
- [ ] **T-121 — Płatność PayU za depozyt** ⛔ **STOI NA PAYU — stan 2026-09-07** (Janek wie, sprawa w toku, **bramki nie ruszamy**): `GET /api/v2_1/paymethods` na produkcyjnym POS oddaje **19 z 19 metod jako `DISABLED`** — ten sam stan co 31.08, mimo że creds są od 06.08. Praktyczny skutek: bramka na stronie jest włączona, więc klient klikający „zapłać depozyt" dostaje odbicie (21.08 zam. 423100, depozyt 6 150 zł — siedem prób pod rząd). Sprawdzenie bez generowania transakcji: samo `paymethods`. Pakiet wykonawczy jest gotowy i nie traci ważności, ale **nie jest to pozycja „gotowa do startu"**, dopóki POS nie wróci. Poprzedni nagłówek „🔵 ODBLOKOWANY 2026-08-06, pakiet wykonawczy gotowy" dotyczył wyłącznie zdobycia danych akceptanta i wprowadzał w błąd co do gotowości. Memory: `project_payu_pos_zablokowany_2026_08_25`. — 27–36 h (`docs/sesje/2026-08-06-T-121-PAKIET-WYKONAWCZY.md`, makiety: `tmp/t121-makiety-payu.html`). **Janek ma produkcyjne creds PayU** (`~/secrets/payu/prod-primaauto.env`, 06.08) — wniosek akceptanta załatwiony, „bloker zgody PayU" z opisu specu jest nieaktualny. Zakres ucięty decyzją Janka: **żadnych cronów** (przypomnienia ani reconkiliacja) — PayU wysyła Ruslanowi własny mail o każdej transakcji, więc brak naszego maila przy obecnym mailu z PayU sam jest sygnałem awarii webhooka. Doszedł **BLIK z kodem wpisywanym na naszej stronie** (`BLIK_AUTHORIZATION_CODE`, +3–4 h) — przy 79,6 % mobile najkrótsza ścieżka; redirect na bramkę zostaje dla kart i portfeli. Widget `payu-widget.js` odrzucony (schyłkowy). Otwarte, nie blokuje: zwrot depozytu przez API vs ręczny przelew; numer i status zamówienia przy kwocie (§8a pakietu). Klient wpłaca zwrotny depozyt online zamiast przelewu ręcznego. Decyzje 27.07: przycisk pojawia się już przy statusie **`potwierdzone` (krok 3 kreatora)** i zostaje do opłacenia; pod nim komunikat „Zgłoszenia z opłaconym depozytem zwrotnym realizujemy w pierwszej kolejności" + „Poczytaj więcej" → `/depozyt/`; kwota z configu, **zamrażana na zamówieniu** (`:452`), ta sama dla pośrednictwa i leasingu. PayU sandbox ograny (`~/secrets/payu/`, damianchen+digit). **Dotyka strefy kruchej `class-asiaauto-order.php`** — wyłącznie `markDepositPaid()`. Spec: `docs/roadmapa/T-121-platnosc-online-depozyt.md`.
- [~] **T-244 — DEFEKT: magic link ginie po pierwszym otwarciu i przy drugim zamówieniu** ⚪ **ZDJĘTE Z KOLEJKI 2026-09-07 (decyzja Janka)** — opis defektu zostaje jako kontekst dla T-209 (mechanizm nadal działa tak, jak opisano). Pierwotnie: 🔴 **ZWERYFIKOWANY NA PRODUKCJI 2026-08-07** (`class-asiaauto-order.php:1428-1479`, v0.34.22) — zgłoszony jako „osobny defekt do kolejki" w pakiecie T-209 z 06.08 i **nigdy nie dopisany**; wyłapane przy przeglądzie 07.08. **2–4 h.** Dwa błędy w jednym mechanizmie: (1) token trzymany w **user_meta, jedno gniazdo na użytkownika** (`_asiaauto_magic_token` + `_expires` + `_order`) — `generateMagicLink()` robi `update_user_meta`, więc link do zamówienia B **kasuje link do zamówienia A**; klient z dwoma zamówieniami traci dostęp do pierwszego; (2) `verifyMagicLink()` woła `clearMagicToken()` **przy udanym użyciu**, czyli link jest **jednorazowy** — ponowne otwarcie maila nie działa. ⚠️ **Ryzyko, które trzeba przetestować przed T-209:** skanery i prefetchery linków w poczcie firmowej (Outlook/Defender, Gmail) potrafią odwiedzić URL zanim zrobi to człowiek — wtedy token zostaje skonsumowany i **klient dostaje martwy link**. T-209 czyni z tego linku „jedyną drogę klienta do zamówienia" i podaje go jako przycisk **i** goły URL, więc ekspozycja rośnie. **Fix: token per zamówienie (post_meta zamiast user_meta) + wielokrotny użyt w oknie TTL 48 h** zamiast kasowania przy pierwszym odczycie. ⚠️ Dotyka `class-asiaauto-order.php` = strefa krucha — addytywnie, z testem regresji na istniejących linkach. **Powiązane z T-209 — sensownie robić przed nim albo razem.**
- [ ] **T-245 — Rozjazd „cena końcowa" vs „kwoty szacunkowe"** 🟡 wykryty przy okazji T-209 (06.08), **też nigdy nie dopisany do kolejki** — na stronie głównej i w schema FAQ mówimy „cena końcowa", a **załącznik nr 2 do umowy** (`class-asiaauto-contract.php:1285`) mówi „kwoty szacunkowe". Rozbieżność między obietnicą marketingową a dokumentem, który klient podpisuje. **Do rozstrzygnięcia z Ruslanem, co jest prawdą**, potem ujednolicenie w jedną stronę. **2–3 h** (audyt wystąpień + korekta treści; jeśli ruszamy wzorzec umowy — strefa krucha, osobna zgoda).
...
---
## 📱 UX MOBILE — T-247 przycisk filtrów w katalogu

- [x] **T-247 — Przycisk filtrów na telefonie: „Filtry" → „Marka / model", czerwony** ✅ **WDROŻONE 2026-08-20** (poza zakresem T-246, zgłoszone przez Janka przy okazji) — pasek mobilny `/samochody/` miał przycisk filtrów jako **najsłabszy wizualnie element**, słabszy niż sortowanie obok, mimo że przy **79,6 % ruchu mobilnego** to główne wejście do zawężania listy. **Pomiar przed zmianą wykazał, że sama zmiana etykiety by się nie zmieściła:** „Marka / model" ma 138 px wobec 80 px („Filtry"), a pasek był pełny, bo `<select>` sortowania rozpychał się do **170 px** — do najdłuższej opcji „Cena: od najwyższej". Licznik ofert ucinał się wtedy na 390 px (42 z 66 potrzebnych), na 360 px praktycznie znikał (12), a **na 320 px ucinał się już przed naszą zmianą** — istniejąca usterka reflow. **Rozwiązanie (pomysł Janka): skrócenie etykiet sortowania** do `Najnowsze / Najtańsze / Najdroższe` — select zwęża się naturalnie, bez ucinania tekstu; dodatkowo `min-width` 130 → 112 px i chowanie samego słowa „ofert" poniżej 400 px (liczba zostaje w całości). Przycisk: tło `--aa-accent`, biały napis (4,85:1); **licznik aktywnych filtrów odwrócony** na białe tło z czerwoną cyfrą, bo czerwony na czerwonym by zniknął. ⚠️ **Pomiar końcowy na 360/320 px NIEDOKOŃCZONY** — Chrome nie dostał zasobów na koncie LVE (blokowały je równoległe sesje Claude Code); rachunkowo 18 px odzyskane z sortowania pokrywa brakujące 14 px na 360 px, ale **to wyliczenie, nie pomiar**. Do domknięcia: `pptr` na 390/360/320 px. Kopie: `.bak-2026-08-20-filtry`. Pliki: `class-asiaauto-inventory.php`, `asiaauto-inventory.css`.

---

## ♿ DOSTĘPNOŚĆ — T-246 wdrożenie poprawek WCAG 2.2 (audyt 13.08.2026)

- [x] **T-246 — Wdrożenie poprawek dostępności (N-1…N-7) + zgłoszenie do reaudytu** ✅ **WDROŻONE 2026-08-20, ~2 h 45 min** — axe na 6 stronach **266 → 15** elementów z błędem (wszystkie 15 to jeden przypadek: czerwień marki jako tekst na jasnoszarym, 4,48 wobec progu 4,50 — **świadomie zachowana** decyzją Janka po przeglądzie wizualnym), przejście klawiaturą `/samochody/` **17 → 0** bez wskaźnika, fokus zasłaniany banerem **5/9/2 → 0/0/0**; 2.1.2 zachowane, reflow 320 px czysty, 0 błędów JS, +4,5 KB wagi. Motyw 1.2.2 → 1.2.3. ⚠️ **Dwie karty audytu okazały się fałszywe** — N-4 (pole wyszukiwarki ma `aria-label`) i N-7 (przycisk ma `value="Zaloguj się"` z `wp_login_form()`); dowód z drzewa dostępności CDP: 0 kontrolek bez nazwy na `/samochody/` (33) i `/klient/` (8), pliki nietknięte po dacie audytu. **N-2 niepotwierdzony** (0 przeskoków nagłówków na 3 stronach) — audytor ma wskazać, które 2 z 16 stron. ⚠️ Reguła CSS z promptu audytora **nie zadziałałaby** — specyficzność (0,2,0) przeciw gaszącej (0,4,1). Zamknięte realnie: N-1 (poza czerwienią marki), N-3, N-5, N-6. ⚠️ **Przegląd wizualny 20.08 cofnął 2 zmiany:** czerwień marki wróciła do `#D63031` (zmieniona była na `#C92A2B` dla 0,02 pkt kontrastu, ale obejmowała `--c-accent` = ceny, przyciski i linki w całym serwisie — koszt nieproporcjonalny do zysku); badge „Nowe” z granatu na ciemny brąz `#4A3800` (5,57) — granat czytał się jako kolor obcy wobec bursztynu, brąz zostaje w rodzinie koloru. **Reguła na przyszłość:** przy brakującym ułamku punktu kontrastu — zgłaszać jako pozycję do decyzji, nie zamykać zmianą koloru marki; liczba jest argumentem, nie rozstrzygnięciem. Raport + prompt zwrotny do reaudytu: `docs/dostepnosc/2026-08-20-wdrozenie-wcag.md`. *(pierwotny opis)* — task założony **20.08.2026** z promptu wykonawczego audytora, żeby robota miała ślad w kolejce projektu (prompt żył dotąd wyłącznie w `auranet` i na dropie auratest). Audyt zamknięty 13.08 (próbka 16 adresów, 55 kryteriów WCAG 2.2) dał **siedem niezgodności**: N-5 wskaźnik fokusu (poważna, 2.4.7+1.4.11), N-1 kontrast tekstu pomocniczego (poważna, 1.4.3 — **617 elementów** w axe), N-7 przycisk logowania bez nazwy dostępnej (4.1.2+3.3.2), N-4 pole wyszukiwarki bez trwałej etykiety (3.3.2), N-6 baner zgód zasłania element z fokusem (2.4.11), N-3 cele dotykowe < 24 px (2.5.8, 25 zgłoszeń), N-2 przeskok poziomu nagłówka (1.3.1, 2/16 stron). ⚠️ **Korekta v1 → v2 (19.08): naprawa siedzi w PLUGINIE, nie w motywie** — w `primaauto2026` nie ma ani jednego `outline: none`, jest poprawne `:focus-visible` w `assets/css/base.css:214`; wszystkie wygaszenia są w `asiaauto-sync` (`asiaauto-inventory.css:104,163,207,239,465` — **cztery na stanie spoczynkowym z `!important`**, stąd pomiar „9 kolejnych elementów bez wskaźnika na `/samochody/`" — plus `asiaauto-order-wizard.css:232`, `class-asiaauto-login.php:181`, `class-asiaauto-shortcodes.php:2759`). Zapis „nie dotykamy `asiaauto-sync`" **nie obowiązuje w warstwie prezentacji**; import, sync i logika zamówień nietknięte. **Metoda: dokładamy silniejszą regułę `:focus-visible` na końcu arkusza, nie usuwamy istniejących `outline: none`** (usunięcie zmienia wygląd pól, dołożenie nie). Strefy ryzyka: `asiaauto-inventory.css` = główna ścieżka ruchu i konwersji (`!important` + `font-size: 0` na przyciskach — łatwo rozsypać filtry na mobile), `asiaauto-order-wizard.css` = ścieżka pieniędzy, baner Complianz = wszystkie strony (zła pętla fokusu łamie 2.1.2, dziś spełnione). Pięć etapów: poprawki → testy wewnętrzne (`php -l`, kopie `.bak-2026-08-19-a11y`, podbicie wersji pluginu, diff dla Janka) → pptr headless (axe/zrzut/reflow/CWV) → **Chrome MCP realną klawiaturą** (test przesądzający o N-5) → raport `docs/dostepnosc/2026-08-19-wdrozenie-wcag.md` + prompt zwrotny do wklejenia w `auranet`. **Godziny nieoszacowane** — to pierwszy przebieg pełnego cyklu audyt → wdrożenie → reaudyt i pomiar czasu per etap jest jednym z celów. **Nie orzekamy zgodności** — orzeka reaudyt po stronie audytora. Spec: `docs/roadmapa/T-246-wdrozenie-dostepnosc-wcag.md`; źródła: `~/projekty/auranet/docs/uslugi/wdrozenia/2026-08-19-primaauto-wdrozenie-v2.md` + audyt `…/audyty/primaauto-2026-08-13.md`.
...
- [ ] **T-204 — Panel klienta (konto zalogowanego) — FUNDAMENT** 🟡 **KOD WDROŻONY 2026-09-08 (plugin 0.40.0), TASK OTWARTY — PANEL NIE DZIAŁA** (decyzja Janka 2026-09-17: dopóki klient nie ma jak wejść, funkcja nie istnieje) — spec: `docs/roadmapa/T-204-panel-klienta-fundament.md`, changelog: `docs/VERSIONS.md` (0.40.0). `/klient/` przestało być atrapą przerzucającą zalogowanego klienta na jego ostatnie zamówienie i jest panelem z czterema zakładkami: **Moje zamówienia** (ożywiony martwy kod), **Moje dane** (edycja + zmiana hasła), **Ulubione** i **Alerty** jako puste sloty pod T-114 i T-188. Doszła **rejestracja i logowanie z frontu** (`/klient/?rejestracja=1`) z honeypotem, limitem 5 prób na IP na godzinę i wymaganą zgodą na regulamin. Nowy plik `includes/class-asiaauto-account.php` — panel dostał własną klasę, bo `class-asiaauto-shortcodes.php` miał 2865 linii, a T-114/T-188/T-249 dołożą kolejne zakładki (przy okazji zeszło z niego 290 linii martwego kodu → 2593). 📌 **Wejście w menu i headerze CELOWO poza zakresem** (krok 6 specu) — decyzja Janka 08.09: logowanie i rejestracja zostają niepodlinkowane, kształt wejścia rozstrzyga **osobny mockup**. Dziś do panelu wchodzi się adresem, po wylogowaniu i po odbiciu z wp-admin. ✅ **Krok 5 specu odpadł jako zrobiony** — hasło plaintextem w mailu naprawił T-244 (odebrany 10.08). 🔒 **Zamrożenie danych umownych (decyzja Janka 08.09):** umowa PDF czyta dane klienta **na żywo z `usermeta`** (`class-asiaauto-contract.php:220`), więc edycja adresu czy PESEL-u po podpisaniu rozjechałaby przyszłą regenerację z dokumentem — telefon i e-mail edytowalne zawsze, dane umowne tylko dopóki żadne zamówienie nie przekroczyło statusu `podpisane` (pomiar w dniu wdrożenia: 18 z 67 klientów zamrożonych). Panel **nie woła** `Contract::regenerate()`. 🔎 **Znalezione przy okazji:** `listing_title` jest puste dla **56 z 206 zamówień**, bo rotacja kasuje ogłoszenia — nazwę auta bierzemy teraz z tytułu zamówienia (`orderLabel()`), inaczej co czwarta pozycja byłaby bezimienna. **Weryfikacja na produkcji:** rejestracja, honeypot, walidacja zgody, zapis danych, zmiana hasła (zła i poprawna), cudze zamówienie → 403, REST → 401, wp-admin → 302 na `/klient/`, **magic link z maila → loguje i ląduje w kreatorze** (test regresji). Konto testowe usunięte. **Odblokowuje T-114, T-188, T-249 i częściowo T-115.** Wycena po fakcie: **15–20 h realnie** zamiast 18–24 (krok 5 odpadł). 🔴 **PRZESTAWIONE NA OTWARTE 2026-09-17 (Janek): panel nie działa.** Kod jest, wejścia nie ma — brak linku w menu i headerze (krok 6 specu świadomie pominięty 08.09), więc do `/klient/` trafia tylko ten, kto zna adres albo odbije się z wp-admin. **Do domknięcia:** rozstrzygnąć kształt wejścia (mockup logowania/rejestracji w headerze i menu mobilnym, 2–3 warianty, desktop i telefon), podlinkować, sprawdzić ścieżkę zalogowania od zera na produkcji. Razem z tym idzie **T-257** (krok „uzupełnij dane do umowy” — mail i ekran).
- [ ] **T-257 — Krok „uzupełnij dane do umowy": mail i ekran zrozumiałe dla klienta** 🆕 **ZGŁOSZONE 2026-09-17 (Janek, sygnał od Ruslana)** — Ruslan twierdzi, że **klienci nie umieją i nie wiedzą, co mają zrobić**, gdy przychodzi moment uzupełnienia danych do umowy. Do sprawdzenia i przepisania **dwie warstwy razem**: (1) **treść maila statusowego** kierującego do uzupełnienia danych — czy tłumaczy, po co te dane, ile to zajmie i co dzieje się dalej, czy przycisk jest jednoznaczny; (2) **ekran po wejściu z linku** — czy jest oczywisty na **telefonie i desktopie** (mobile = 79,6% ruchu), czy przy polach wrażliwych (PESEL, adres) jest powód ich podania, czy komunikaty błędów prowadzą zamiast blokować. Kotwice w kodzie: `class-asiaauto-order-wizard.php:39,243` (krok „Dane do umowy"), `class-asiaauto-order-content.php:193,526` (narracja kroków i statusów), `class-asiaauto-order-api.php:482` (komunikat po częściowym zapisie). ⚠️ **Panel klienta z T-204 NIE jest uruchomiony** (potwierdzenie Janka 17.09) — kod wdrożony 08.09 (0.40.0), ale wejście w menu i headerze celowo poza zakresem, klient wchodzi dziś wyłącznie magic linkiem z maila; **ten task robimy razem z powrotem do panelu klienta i rejestracji**, nie osobno. ⚠️ Zmieniony szablon maila **musi** przejść `scripts/t209_build_tresci.py` → `t209_export_templates.py` → opcja `asiaauto_order_email_templates` — wpis w `defaultEmailTemplates()` to fallback sprzed T-209 i wygląda jak z innego systemu. ⚠️ Zmiany wizualne = **makieta stanu obecnego + 2–3 warianty**, desktop i telefon, wdrożenie po wyborze. 📌 **Decyzja Janka 17.09: bez rozpoznania teraz** — zbieramy przy powrocie do tematu panelu i statusów. Wycena po rozpoznaniu.
- [ ] **T-256 — Przepięcie płatności depozytu PayU → Tpay** 🟡 **W TOKU, 6 h z 12 h** — 16.09: porównanie operatorów (`docs/biznes/2026-09-16-bramka-platnosci-porownanie.md`), konto Tpay założone, integracja v0.41.0 przetestowana na sandboxie, flaga OFF. Dalej: weryfikacja konta (przelew Ruslana) → panel Tpay → płatność 1 zł na prod → włączenie. Prompt: `docs/sesje/2026-09-16-PROMPT-tpay-weryfikacja-i-przepiecie-produkcja.md`
- [x] **T-255 — Tuning `/marki/`: galeria z własnych sesji, porządek w siatce, SEO** ✅ **WDROŻONE 2026-09-08, motyw 1.5.0** — pakiet: `docs/sesje/2026-09-08-PROMPT-T255-tuning-marki.md`, baseline pomiaru: `docs/seo/2026-09-08-t255-marki-baseline.md`. **Galeria przestała gnić:** kadry wskazywały na zdjęcia OFERT, a oferta wypada z rotacji po 7 dniach i zabiera plik — **2 z 6 były martwe** (LEVC L380, Voyah Dream), a ten drugi był `og:image` strony. Teraz 9 kadrów z **naszej sesji w Rzeszowie** (`uploads/asiaauto/sesje/2026-08-rzeszow/`, poza drzewem importera), renderowanych z szablonu, linkujących do **hubów modeli** (9 × HTTP 200). **Siatka:** sortowanie malejąco po liczbie ofert zamiast alfabetycznie (Aistaland z 3 autami otwierał sekcję przed Geely ze 111), próg ≥100 zamiast `array_slice(0, 8)` tnącego między Hongqi 162 a Zeekr 154, ogon ≤3 ofert (13 marek) w osobnej grupie `--mini`. **SEO:** title i description dynamiczne (transient dobowy) — opis mówił „50+ marek" z 25.08 przy 51 w siatce; focus keyword na **`marka chińskich aut`** (222 imp/mc, poz. 8,3, **0 kliknięć**); answer-first pod klaster „ile jest chińskich marek" (5 fraz, 80 imp/mc, poz. 8,3–10,6, CTR 0% — odpowiedź była w zwiniętym `<details>` na dole). **Bez logotypów marek** (decyzja Janka 08.09 — znaki towarowe; klaster „logo" ~79 imp/mc świadomie odpuszczony, potwierdza zdjęcie T-225c z 04.08). Klasy `.aa-brand-card`/`.aa-brand-grid` współdzielone z hubami **nietknięte** — diff CSS to 0 usunięć i 0 modyfikacji. Pomiar po: 0 obrazów bez ładowania (390/1366 px), brak przewijania poziomego, transfer ~1,67 MB. **Domyka T-225e.** Zgłoszone do indeksacji. ~6 h.

- [ ] **T-249 — Dokumenty w strefie klienta (wymiana plików Ruslan ↔ klient + lista oczekiwanych dokumentów)** 🆕 **ZGŁOSZONE 2026-09-02 (Janek)** · spec: `docs/roadmapa/T-249-dokumenty-strefa-klienta.md` · **45–60 h realnie / 110–145 h rynkowo** — Ruslan dodaje do zamówienia dowolny plik z własnym opisem, klient tak samo, a zamówienie pokazuje obu stronom **listę dokumentów oczekiwanych** (kto dostarcza, na jakim etapie, czy wpłynął, czy przyjęty). Dziś wymiana idzie mailem i WhatsAppem: system przyjmuje od klienta **dokładnie dwa pliki** (`signed_contract`, `payment_proof`) w oknie dwóch statusów (`umowa_gotowa`/`podpisane`) i **bez pola na opis**, a Ruslan może wgrać **wyłącznie umowę PDF** (`upload_custom_contract`, wymaga numeru umowy). Po podpisaniu klient nie ma żadnego kanału — a **krok 5 kreatora istnieje** (`class-asiaauto-order-wizard.php:670`, statusy `zarezerwowane`…`zakonczone`) i stoi pusty. 🔴 **Pomiar 02.09, niezależny od samej funkcji: dokumenty są dostępne bez logowania.** 68 załączników przy zamówieniach (62 PDF, 5 JPG, 1 DOCX) w `uploads/contracts/` — bezpośredni URL oddaje plik (HTTP 206), a WordPress dogenerował **miniatury JPG pierwszej strony każdej umowy** (170 plików przy 62 PDF). Chroni je tylko 8-znakowy hash w nazwie = *security by obscurity*, przy dokumentach z PESEL i podpisem odręcznym. Co działa: listing katalogu 403, `wp/v2/media` 401, CPT `asiaauto_order` niepubliczny, `canAccessOrder()` autoryzuje poprawnie. ⚠️ **Wymaga T-204** (panel klienta to dziś atrapa) i **T-244** przed uruchomieniem powiadomień (link z maila to jedyna droga klienta). ⚠️ **Kolizja plikowa z T-218 i T-209** — ten sam `class-asiaauto-order-admin.php` (3090 linii) i `class-asiaauto-order-mail.php`; nie robić równolegle. ⚠️ `class-asiaauto-order.php` = **strefa krucha** — status dokumentu jest metadaną opisową, **nigdy warunkiem przejścia zamówienia** (inaczej brak skanu blokuje sprzedaż); metoda addytywna, nowa klasa `class-asiaauto-documents.php` obok istniejących. **Decyzja Janka 02.09: jedna bryła po T-204** (wariant z podziałem na 3 fazy odrzucony). Otwarte: katalog dokumentów do ustalenia z Ruslanem (faza 0 — **najpierw quiz do Janka**), retencja po zakończeniu zamówienia (razem z T-221), limity objętości plików.

- [x] **T-253 — Termin dostawy edytowalny w karcie zamówienia** ✅ **WDROŻONE 2026-09-08, v0.39.3** — pole obok VIN-u z podglądem wyliczonej daty, puste = fallback z konfiguracji; zweryfikowane na PDF obu wzorców (pośrednictwo „w terminie 90 dni”, leasing „do 90 dni”, pusta meta → 120). Przy okazji ujednolicony rozjazd fallbacku 60/120 w **siedmiu** miejscach (wizard ×4, api ×1, JS ×2 — te dwa poza pakietem, a to liczba widziana przez klienta). Pierwotne zgłoszenie 2026-09-04 (Janek) — liczba dni dostawy w umowie ma być **podpowiadana** z ustawień, ale nadpisywalna **w konkretnym zamówieniu**. Dziś, żeby wystawić umowę z innym terminem, trzeba zmienić `default_delivery_days` globalnie w ustawieniach, wygenerować PDF i przywrócić wartość — przy równoległych zamówieniach ryzyko wystawienia umowy z cudzym terminem. ✅ **Rozpoznanie 04.09: dane per zamówienie JUŻ ISTNIEJĄ, brakuje wyłącznie pola w UI.** Meta `_order_delivery_estimate` (`class-asiaauto-order.php:128`) jest zapisywana **raz**, przy zakładaniu zamówienia, kopią z konfiguracji (`:518`) i **nigdzie nie da się jej zmienić** — w całym pluginie jest tylko ten jeden zapis i odczyty. Umowa czyta już właściwą ścieżkę: `class-asiaauto-contract.php:301` bierze `delivery_estimate` z zamówienia, a config jest wyłącznie fallbackiem — czyli po dołożeniu pola PDF zadziała bez dotykania szablonów (§4 ust. 1 pośrednictwo, `:848`; §4 leasing, `:1380`). **Metoda: dokładnie wzorzec `_order_vin` z T-220** — pole w karcie zamówienia + zapis w `class-asiaauto-order-admin.php:283`, wartość z ustawień jako placeholder/domyślna, puste = fallback do konfiguracji (zachowanie dzisiejsze). ⚠️ `class-asiaauto-order.php` = **strefa krucha** — addytywnie, bez ruszania statusów i przejść; termin jest metadaną opisową, nigdy warunkiem przejścia. ⚠️ **Kolizja plikowa z T-249, T-218 i T-209** — ten sam `class-asiaauto-order-admin.php` (3090 linii); nie robić równolegle. 📌 **Znalezione przy okazji, do domknięcia w tym samym ruchu:** fallback jest rozjechany — 120 dni w `class-asiaauto-order.php:371`/`:518` i `class-asiaauto-contract.php:301`, ale **60 dni** w `class-asiaauto-order-wizard.php:465,559,612,641` i `class-asiaauto-order-api.php:194`, więc klient w kreatorze widzi inną liczbę niż ta, która trafi do umowy. **1,5–2 h realnie / 4–5 h rynkowo** (wycena domknięta 04.09 — doszedł podgląd daty w karcie, naprawa rozjazdu 60/120 w pięciu miejscach i test obu wzorców umowy). 📦 **PAKIET WYKONAWCZY: `docs/sesje/2026-09-04-PROMPT-T248-T253-numer-przesylki-i-termin.md`** — robione jednym wejściem razem z T-248 (ten sam plik). ✅ Pomiar bazy 04.09: **194 zamówienia, wszystkie z wartością 120** — brak zamówień bez meta, brak wartości odstających. Na stronie postępu jako pozycja „do zrobienia”.

- [x] **T-248 — Numer przesyłki w zamówieniu + informacja o transporcie do klienta** ✅ **WDROŻONE 2026-09-08, v0.39.3–0.39.5** — pole numeru w karcie (`META_TRACKING_NUMBER`), odnośnik z szablonu `tracking_url_template` (Maersk), przycisk wysyłki z guardami, szablon `tracking_sent` znormalizowany ścieżką T-209 jako **siedemnasty** w opcji, numer w panelu klienta jako **osobna karta „Transport”** na granatowym tle (0.39.5), widoczna gdy numer jest wpisany — **niezależnie od kroku kreatora** (0.39.4; wcześniej ginęła w kroku 5, którego nie widać przy statusie „potwierdzone”). Kontrasty WCAG 2.2 AA zmierzone: 8,05:1 / 14,22:1 / 9,86:1 / 13,15:1, fokus 14,22:1. Automat armatora poza zakresem → T-254. Pierwotne zgłoszenie 2026-08-24, zakres przedefiniowany 2026-09-04 (Janek)
> ⚠️ **Wnioski z wdrożenia (do kolejnych maili):** (1) nowy szablon maila **musi** przejść `t209_build_tresci.py` → `t209_export_templates.py` → opcja `asiaauto_order_email_templates`; wpis w `defaultEmailTemplates()` to fallback sprzed T-209 i wygląda jak z innego systemu — Janek dostał taki mail, bo przycisk wdrożyłem przed treścią; (2) każda nowa wartość przykładowa z makiety wymaga wpisu w `REPLACEMENTS` eksportera — inaczej idzie do klientów na sztywno (złapane: `MAEU7841239`); (3) **`class-asiaauto-order-admin.php` jest znów wolny** — czekają na niego T-249, T-218, T-209 — pole na numer kontenera/listu przewozowego w karcie zamówienia, przycisk „wyślij informację o transporcie” (mail do klienta w naszym wrapperze) oraz numer z odnośnikiem do śledzenia w panelu klienta. 🔄 **Automat przewoźnika wypadł z tego taska** — odpytywanie Maerska o pozycję/port/ETA przeniesione do T-254 (parking). ✅ **Rozpoznanie 04.09:** numeru przesyłki **nie ma w kodzie w żadnej postaci** (grep po `tracking|kontener|przesyłk` w `includes/` pusty); statusów `w_drodze` jest **27**, `na_placu` **23** — funkcja ma na czym pracować od razu. **Panel klienta NIE czeka na T-204**: krok 5 kreatora (`class-asiaauto-order-wizard.php:382`) już renderuje pasek transportu i ramkę „szacowany czas dostawy”, a klient wchodzi tam magic linkiem; wystarczy wiersz z numerem (`asiaauto-order-wizard.js:686` + payload `class-asiaauto-order-api.php:361`, autoryzacja `canAccessOrder()` poprawna). **Metoda: wzorzec T-220 (aneks VIN)** — `case` w switchu za istniejącą bramką nonce+`ORDER_CAP` (`class-asiaauto-order-admin.php:120`), handler wzorem `sendAnnexVinEmail()` (`:735`), znacznik wysyłki jak `META_ANNEX_VIN_SENT_GMT`. 📌 **Decyzja D-1 (Janek 04.09): w mailu do klienta nie ma linku do przewoźnika** — sam numer + przycisk do naszego panelu; link do śledzenia widzi Ruslan w adminie i klient w panelu. Szablon URL w konfiguracji zamówień (`tracking_url_template`), więc kolejny armator = podmiana wartości, nie deploy. ⚠️ **Pułapka wrappera maili:** `AsiaAuto_Order_Mail::wrapHtml()` czyta `self::$context` ustawiany tylko przez hooki statusowe — przy wysyłce z panelu trzeba wywołać `setContext()`, inaczej mail wychodzi bez nagłówka i paska postępu. *Zauważone obok:* `sendAnnexVinEmail()` z T-220 ma dokładnie ten brak — do decyzji osobno. ⚠️ Szablon maila musi powstać przez `scripts/t209_build_tresci.py` → `t209_export_templates.py` (w bazie leży 16 gotowych szablonów HTML; ręcznie pisany tekst odstaje wyglądem). ⚠️ `class-asiaauto-order.php` = **strefa krucha** — addytywnie, bez ruszania statusów; numer jest metadaną opisową. ⚠️ **Kolizja plikowa z T-253, T-249, T-218 i T-209** — ten sam `class-asiaauto-order-admin.php`; T-253 robione tym samym wejściem, reszta czeka. **4–4,5 h realnie / 10–13 h rynkowo** (wycena 04.09 po rozpoznaniu; stare „3 h realnie / 7–9 h rynkowo” dotyczyło innego zakresu i nie obejmowało ani maila, ani panelu klienta). 📦 **PAKIET WYKONAWCZY: `docs/sesje/2026-09-04-PROMPT-T248-T253-numer-przesylki-i-termin.md`**. ✅ **PRZEWOŹNIK USTALONY 2026-09-07 (Janek): MAERSK.** Szablon do `tracking_url_template`: `https://www.maersk.com/tracking/{numer}` — **adres niezweryfikowany**, Maersk odbija curl błędem 502, więc przed wdrożeniem otworzyć w przeglądarce z prawdziwym numerem kontenera i potwierdzić postać (sam numer vs `#tracking=`). Przypominam decyzję D-1: **w mailu do klienta linku do przewoźnika nie ma** — adres śledzenia widzi Ruslan w adminie i klient w panelu. Powiązane: **T-254** (automat Maerska: pozycja statku, port, ETA) zostaje na parkingu. ~~Do ustalenia z Ruslanem przed wysyłką pierwszego maila:~~ jakim przewoźnikiem chodzą przesyłki i jaki jest adres śledzenia (do szablonu URL) — samo pole i panel powstają bez tej odpowiedzi.

- [ ] **T-254 — Automat armatora: pozycja statku, następny port i ETA przy statusie „w drodze”** ⏸️ **PARKING — wydzielone z T-248 2026-09-04** — po wpisaniu numeru przesyłki system sam odpytuje przewoźnika i pokazuje w zamówieniu aktualne dane transportu, odświeżane cronem, żeby przesunięcie terminu przez armatora docierało do klienta bez telefonu do Ruslana. **Nierozpoznane:** zasady dostępu Maersk do danych śledzenia (API czy tylko strona), koszt, limity, czy poza Maerskiem chodzą inni armatorzy (każdy kolejny = osobna praca). **Wymaga T-248** (numer musi być gdzie zapisać). **Wycena dopiero po sprawdzeniu dostępu do danych** — dla porządku: to integracja z cache i cronem, nie pole w formularzu, więc rząd wielkości jest inny niż T-248.

- [ ] **T-243 — Dystrybucja treści na kanały społecznościowe (parasol, 8 pozycji)** ⏸️ **ODŁOŻONY 2026-09-07 (decyzja Janka)** — dystrybucję prowadzimy **ręcznie**, a kontrolę daje **dashboard pomiaru** (`https://auratest.pl/pa-live-8aaf08d6ece0db176603/`): kampanie, koszt, wyświetlenia, kliknięcia i kontakty z Meta, Google Ads i GA4 w jednym miejscu, plus podglądy kreacji przez `scripts/social/podglady.py` dla kogoś bez konta na FB. Automat wypychający treść na kanały nie jest dziś potrzebny — wracamy do niego, gdy skala ręcznej roboty zacznie przeszkadzać. *(stan sprzed odłożenia)* — instrukcja dostępów u Ruslana, faza a blokuje część e, a w kolejce stoją taski gotowe do wykonania (T-121, T-209). Wracamy, gdy Ruslan odpisze albo gdy zwolni się miejsce. 🔵 **SPEC GOTOWY 2026-08-07** — `docs/roadmapa/T-243-social-dystrybucja-tresci.md`. **89–119 h realnie / 225–300 h rynkowo; pierwsza fala 57–75 h; zużyte 4 h** (rozpoznanie + spec + instrukcja, 07.08 — w changelogu postępu). Produktem jest **dystrybucja, nie treść**: materiał ma natywnie istnieć na każdej platformie, bo dopiero wtedy jest czym promować (film na YT → kampania wideo, post na FB → podbicie, rolka → Spark Ads). Ruslan wskazuje auto w panelu przy ofercie → system przygotowuje media (ffmpeg/magick: kadry, WebP z nazwami pod frazy, miniatury z ceną), pisze copy per platforma (`claude -p`, abonament) i wypycha jako **zaplanowane** na FB/IG/YT/TikTok; Ruslan zarządza natywnie w Business Suite i Studio, my trzymamy wspólny rejestr, bo Janek nie ma dostępu do kanałów. **Mechanizm: pipeline modułów, NIE agent** (D1) — publikacja jest nieodwracalna i zewnętrzna, ograniczenia są bramkami a nie instrukcją, 5 z 6 kroków to czysta mechanika, konto siedzi na limicie LVE. Kod w `scripts/social/` wzorem `scripts/kb/`, ekran `class-asiaauto-admin-social.php` wzorem `-che168-import.php`, dashboard statycznym `build.py --deploy` wzorem kosztorysu. **Wchłania T-241** jako pierwszy przebieg fazy c. **Fact-check liczbowy wchodzi od razu** — post rozchodzi się szerzej niż hub i nie da się go skorygować po publikacji (T-208 nazywa ten brak największym ryzykiem pipeline'u contentowego).
...
- [~] **T-115 — Porównywarka aut** 🟢 **WDROŻONE 16.09 w v0.42.0, niepodlinkowane, noindex** — `/porownywarka/` i `/porownywarka/a-vs-b/`. Spec: `docs/roadmapa/T-115-porownywarka.md`, changelog: `docs/VERSIONS.md` (0.42.0). Porównujemy **wersje** (specid; tabela `wp7j_asiaauto_versions`, 807 wersji, cron 05:15), wzór cpubenchmark, max 3, bez logowania, sekcje karty oferty 1:1, słownik, telefon = przewijana tabela. **Etap 2:** przycisk na karcie oferty, wejście w nawigacji, decyzja o indeksie po danych z logu porównań.
...
- [x] **T-225a — `/marki/`: wstęp + mapa koncernów + klasyfikacja marek** 🟢 **ZAMKNIĘTE 2026-09-07** — przemiar pokazał, że **wstęp i mapa koncernów były już na produkcji od 25.08** (`post_content` strony 263572: 8 516 znaków, 4×H2, render potwierdzony HTTP 200). Z „9 brakujących opisów" zostało **zadanie bezprzedmiotowe**: `term->description` nie jest renderowane nigdzie na froncie — T-225b zastąpił teaser z opisu etykietą koncernu, a hub marki bierze `_asiaauto_lead` + `asiaauto_wiki_body`; `description` jest już tylko lustrem dla analizy RankMath (`class-asiaauto-rest-hub.php:92`). **Realny brak był gdzie indziej — w klasyfikacji marek** (`scripts/set-brand-origin-group.php`, 5 wpisów, apply idempotentny, diff = dokładnie 5 zmian): **BMW bez `origin` stało w siatce „Pozostałe chińskie marki"** (teraz `global_jv`, poza siatką — hub nietknięty), Great Wall bez `origin` (→ GWM), Stelato → „BAIC i Huawei", Aistaland → „GAC i Huawei", Changan Qiyuan → „Changan". Siatka: 55 kafli, wszystkie z grupą kapitałową tam, gdzie marka nie jest własnym koncernem. Marki Ford/Toyota/Honda/Mercedes/BMW **świadomie bez opisu** — 1–4 oferty, opis nie zwróciłby czasu.
...
## T-191 — Domapowanie modeli z popytem (huby bez wpisu w mapowaniu) 🔵 GOTOWE DO STARTU

> **AKTUALIZACJA 2026-09-07 — przemiar obu map na produkcji** (`brand-mapping-v6.1.php` 340 wpisów, `che168-model-map.php` 161). Z 27 pozycji audytu **6 weszło przy okazji innych sesji**: smart #1, smart #3, Geely Galaxy M9, Li Auto MEGA, Leapmotor B01, Ford EVOS. **Zakres zawężony do 6 pozycji, przy których jest dziś podaż** — reszta czeka na powrót ofert:
> 🟢 **P1 WYKONANE 2026-09-07** — 7 kluczy w `brand-mapping-v6.1.php` (340 → 347), `.bak-2026-09-07-t191`, weryfikacja ścieżką produkcyjną (adapter + `getEuForCn`, bez importu): 7/7 trafia w istniejące huby, żaden tytuł nie dubluje marki, regresja na 5 parach sprzed patcha czysta. **Zmierzona pułapka:** dla che168 wpis w `che168-model-map.php` **nie wystarcza** — `canonicalKeyForSource()` szuka bliźniaka w brand-mappingu i bez niego kończy fallbackiem, więc wszystkie pozycje idą do jednego pliku. Klucze nie były przewidywane: dongchedi z `wp asiaauto inspect` na żywych ofertach, che168 z sondy `getOffers` na żywym strumieniu.
> **BYŁO DO ZROBIENIA (P1, jest podaż, import leciał fallbackiem):** Zeekr 8X (7 ofert; term nazywa się gołe „8X") · Hongqi H9 (7 ofert na termie „H9" — **kolizja slugów z Haval H9**) · Haval H9 (2) · Changan UNI-K (2 + 1 iDD) · Ford Bronco (3) · Yangwang U9 (1; w mapie są U7/U8).
> **CZEKA NA PODAŻ (P2, hub istnieje, 0 ofert — wpis dziś nic nie zmienia):** Tank 500 + Hi4-Z · HiPhi Z · Geely Galaxy M7 · Changan UNI-T · Leapmotor C01 · Jetour X90 PLUS (w mapie tylko X90 **PRO**) · NIO EC7 · NIO ES7 · MG Cyberster · Mazda MX-5 · Toyota Corolla Cross.
> ⚪ **ZDJĘTE CAŁKIEM 2026-09-07 (decyzja Janka)** — EU-marki bez podaży i bez termu w bazie, wpis „na zapas" pod ręczne importy: **Nissan Qashqai (165 000/mc), Nissan X-Trail (40 500), Nissan Pathfinder (8 100), MG 6 (1 600), Nissan Teana (320)**. Nie wracamy do nich, dopóki takie auto realnie nie wjedzie.
> ⚠️ Rename termów („8X" → marka w nazwie, rozplątanie H9) **nie wchodzi w T-191** — nazwa termu `serie` idzie do tytułu odświeżanej oferty ([[reference_nazwa_termu_serie_idzie_do_tytulu_oferty]]), więc to osobna zgoda i osobny pomiar.
...
**Grupa B → `che168-model-map` (EU-marki, ręczne importy):** ~~Nissan Qashqai (165 000/mc)~~ · Toyota Corolla Cross (49 500) · ~~Nissan X-Trail (40 500)~~ · Ford Bronco (27 100) · Mazda MX-5 (22 200) · ~~Nissan Pathfinder (8 100)~~ · MG Cyberster (6 600) · ~~MG 6 (1 600)~~ · Ford EVOS (480) ✅ zmapowany · ~~Nissan Teana (320)~~. (Przekreślone = zdjęte 2026-09-07, patrz aktualizacja u góry sekcji.)
