...
```
T-221 (regulamin) ──→ wniosek do PayU ──→ T-121 (płatność online)
...
- [ ] **T-236 — Audyt twierdzeń o homologacji i procesie importu w treściach hubów** 🔴 **PILNE (ryzyko prawne + wyciek know-how)** — 2026-08-04 wykryto na produkcji nieprawdziwy opis procedury: *„homologacja indywidualna przeprowadzana przez Prima-Auto w polskiej stacji diagnostycznej"*. Weryfikacja u źródła ([TDT](https://www.tdt.gov.pl/zalatw-sprawe/homologacja-pojazdow/swiadectwo-krajowego-indywidualnego-dopuszczenia-pojazdu/), biznes.gov.pl): dopuszczenie jednostkowe to **decyzja Dyrektora TDT**, stacja diagnostyczna robi wyłącznie badanie techniczne. ✅ **22 pola naprawione** (`scripts/fix-homologacja-tresci.php`, backup `~/backups/primaauto/2026-08-04/`), zero wzmianek o stacji, FAQ parsuje się na 9 hubach. ⚠️ **ZOSTAJE do decyzji** — audyt 2037 pól z „homologacj" pokazał: **134** „koszt wliczony w cenę", **133** terminy („4-6 tygodni"), **115** „auto odbierasz z polskimi tablicami", **105** „przeprowadzamy homologację indywidualną", plus opisy łańcucha z miastami zespołu w Chinach (Kanton, Shenzhen, Foshan), portami (Gdańsk, Gdynia) i kolejnością etapów. **Blokada:** nie wiemy, jak proces wygląda naprawdę — [[Janek 2026-08-04]] „dopuszczenie pewnie w Polsce, ale uzyskanie dokumentów może odbywać się w innym kraju"; TDT ma osobną procedurę uznania świadectwa z innego państwa UE. **Pytania do Ruslana:** (1) w jakim kraju auto uzyskuje dopuszczenie; (2) kto jest wnioskodawcą; (3) czy klient odbiera auto zarejestrowane (pełnomocnictwo?); (4) co dokładnie zawiera cena. **Zasada docelowa (decyzja Janka):** publikujemy REZULTAT dla klienta, nie OPIS PROCESU — konkurencja nie ma dostawać instrukcji.
...
- [ ] **T-225b — `/marki/`: 3 sekcje, TOP wg `count`, teasery z 49 istniejących opisów** ⚠️ **D1 do potwierdzenia** (czy `global_jv` zostaje). ZAWSZE PYTAJ. 3–4 h.
- [ ] **T-232 — Aktualności: cron 06:45 stoi od 22.07** — diagnoza i wznowienie. 1–2 h.
- [ ] **T-229 — Szablon rankingu + generator z bazy** — `ItemList`, gate kanibalizacyjny GSC, obsługa pułapek (CLTC vs WLTC; hybrydy mieszają zasięg EV z łącznym). 5–6 h.
- [ ] **T-230 — Pierwsze 3 rankingi:** terenówki (~3450/mc) → pickupy (~2480) → vany i 7-osobowe (~2140). 4–5 h.
...
- [ ] **T-225c — `/marki/`: logotypy marek + rejestr licencji** — „logo chińskich samochodów" poz. 7,4 przy zerze obrazków. 3 h.
- [ ] **T-225d — `/marki/`: FAQ + `ItemList`/`FAQPage` + sekcja „Najnowsze rankingi" + indeksacja i baseline.** 2–3 h.
