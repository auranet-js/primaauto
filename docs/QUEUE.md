...
- [ ] **T-233a — Słownik: 15 haseł o pokryciu >1000 aut** 🔵 W TOKU — zawieszenie (wielowahaczowe 1449, McPherson 958, dwuwahaczowe 886, pięciowahaczowe 345, regulacja twardości/wysokości 579), hamulce (tarcze wentylowane 2001, EPB 2029), struktura nadwozia (samonośna 1933 / **ramowa 103** = terenówki), mapy Amap 857, radar mmWave 1061, skrzynia stałoprzełożeniowa w EV 1079, wtrysk bezpośredni 1101, turbo 806. Pipeline `scripts/kb/wiki_generate.py` + nowy `wiki_tier5.json`. ⚠️ warianty zapisu CN (`双叉臂式独立悬架` 682 vs `…悬挂` 204) muszą trafić do `term_keys`, inaczej hasło złapie połowę aut. 6–8 h.
- [ ] **T-226 — Dane podstawowe na ofercie: marka, model, wersja, kolory, napęd, liczba miejsc** — sekcja otwiera się dziś wymiarami (Rok modelowy → Typ nadwozia → Długość…), brak identyfikacji pojazdu. 3–4 h.
- [ ] **T-227 — Parametry jako linki do hubów i katalogu** — marka → hub marki, model → hub modelu, nadwozie/paliwo/kolor/napęd → katalog z filtrem; `noindex` na kombinacjach. (Parametry techniczne obsługuje autolinker — poza zakresem.) 3–4 h.
...
- [ ] **T-234 — Słownik: sekcje „auta z tą technologią" pod hasłami** (ostatni otwarty punkt z T-214). 3–4 h.
