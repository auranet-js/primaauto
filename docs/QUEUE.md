...
- [ ] **T-229 — Szablon rankingu + generator listy z bazy** 🔵 **SPEC GOTOWY 2026-08-04** — `docs/roadmapa/T-229-szablon-rankingu.md`, 5 zadań, 5–7 h. **Decyzje:** ranking dotyczy NASZEJ oferty („co da się kupić w Polsce"), nie sprzedaży w Chinach — mamy komplet parametrów i ceny w PLN, czego nie ma nikt na rynku PL; blok danych między znacznikami `<!--RANKING:START/END-->` **odświeżalny bez przepisywania tekstu** (magazyn zmienia się codziennie); zasięg w **WLTC**, nie CLTC; kryterium = jeden mierzalny parametr, nie „najlepszy"; jedna pozycja = jeden MODEL. ⚠️ **Znaleziska techniczne:** autolinker działa tylko dla kategorii `aktualnosci` (`class-asiaauto-autolink.php:61`) — rankingi nie dostaną linków do słownika bez zmiany; schema `NewsArticle` też przypięta tylko do `aktualnosci`, brakuje `ItemList`. ⚠️ **Pułapki danych:** hybrydy mają w polu zasięgu wartość ŁĄCZNĄ (Denza Z9 GT pokazuje 1036 km), filtr po nazwie paliwa łapie PHEV jako EV (ciąg „(PHEV)" zawiera „EV"), CJK w bazie w dwóch formach. **Podaż zmierzona:** terenówki 102 auta/23 modele, minivany 114/20, kombi 26/4, pickupy 6/6 — przy pickupach tekst musi nazwać wprost, że to cały segment, nie szeroki wybór.
