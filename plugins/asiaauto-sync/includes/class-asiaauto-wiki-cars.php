...
class AsiaAuto_Wiki_Cars {
    ...
    public static function buildIndex(): array {
        ...
        // Filtr wyszukiwarki dla hasła — liczony RAZ przy budowie indeksu (co 6 h), nie przy renderze.
        foreach ($terms as $slug => $keys) {
            if (isset($index[$slug])) $index[$slug]['filtr'] = self::filtrDlaHasla($keys);
        }
    ...
    /**
     * Hasło → query string wyszukiwarki zaawansowanej (T-250 krok 4, 2026-09-03).
     *
     * Dopasowanie po kluczach `extra_prep`: jeśli klucz hasła jest jednym z kluczy flagi
     * w `AsiaAuto_Specs_Table::FLAGS`, CTA prowadzi do `/wyszukiwarka/?wyposazenie=<flaga>`
     * zamiast do całego katalogu. Wyjątki (enumy i klucze liczone pochodnie) są wypisane
     * jawnie — jest ich kilka i lepiej, żeby było je widać, niż zgadywać regexem.
     *
     * Zwraca '' gdy hasło nie ma odpowiednika w filtrach (wtedy CTA zostaje na /samochody/).
     */
    public static function filtrDlaHasla(array $keys): string
    {
        $klucze = array_keys($keys);
        if (!$klucze) return '';
        $pierwszy = (string) $klucze[0];
        $wartosc  = (string) ($keys[$pierwszy] ?? '');

        // 1. wyjątki: enumy wyszukiwarki i flagi liczone z innej kolumny
        if ($pierwszy === 'skylight_type' && str_contains($wartosc, '全景')) return 'wyposazenie=roof_panorama';
        if ($pierwszy === 'air_suspension')     return 'wyposazenie=air_susp';
        if ($pierwszy === 'variable_suspension') return 'zawieszenie=adaptacyjne';
        if ($pierwszy === 'seat_material') {
            $u = class_exists('AsiaAuto_Specs_Table') ? AsiaAuto_Specs_Table::upholstery(['seat_material' => $wartosc]) : null;
            return $u ? 'tapicerka=' . $u : '';
        }
        if ($pierwszy === 'fuel_form') {
            // hasła o rodzajach napędu → filtr „Silnik" (增程式 EREV, 插电 PHEV, 纯电动 EV)
            foreach (['增程' => 'erev', '插电' => 'phev', '纯电' => 'electric', '油电' => 'hybrid'] as $cn => $slug) {
                if (str_contains($wartosc, $cn)) return 'paliwo=' . $slug;
            }
            return '';
        }
        if ($pierwszy === 'sound_brand' && $wartosc !== '') {
            $b = class_exists('AsiaAuto_Specs_Table') ? AsiaAuto_Specs_Table::soundBrand(['sound_brand' => $wartosc]) : null;
            return $b ? 'audio=' . $b : 'wyposazenie=sound_premium';
        }

        // 2. flagi wyposażenia po kluczu (także po prefiksie: `vice_screen_size_15.7`)
        if (class_exists('AsiaAuto_Specs_Table')) {
            foreach (AsiaAuto_Specs_Table::FLAGS as $flaga => $spec) {
                foreach ($spec['keys'] ?? [] as $k) {
                    if ($k === $pierwszy) return 'wyposazenie=' . $flaga;
                }
                foreach ($spec['prefix'] ?? [] as $pre) {
                    if (strncmp($pierwszy, $pre, strlen($pre)) === 0) return 'wyposazenie=' . $flaga;
                }
            }
        }
        return '';
    ...
    public function render(array $atts = []): string {
        ...
        <section class="aa-hub__latest-wrap pa-kb__cars">
            <div class="aa-home__section-header">
                ...
                <?php /* T-250 krok 4: hasło z odpowiednikiem w filtrach prowadzi do wyszukiwarki
                         z zaznaczonym filtrem; reszta zostaje na katalogu. */
                $cta_url  = $data['filtr']
                    ? home_url('/wyszukiwarka/?' . $data['filtr'])
                    : home_url('/samochody/');
                $cta_text = $data['filtr'] ? 'Wszystkie auta z tą funkcją →' : 'Wszystkie auta →'; ?>
                <a href="<?php echo esc_url($cta_url); ?>" class="aa-home__section-link"><?php echo esc_html($cta_text); ?></a>
            </div>
